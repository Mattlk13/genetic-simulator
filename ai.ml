...
let popsize = 20
let mapsize = 300
let population = Population.create 100 5 25 20 20 1 (5,5) (10,10) 0x00FF00
...
let learn () =
  ...
  while true do
    for iteration = 0 to nb_iterations - 1 do
      ...
      let best_ones = Battle.make_play !groups group1_size population popsize mapsize in
      let k = ref 0 in
      let groups2 = ref [] in
      ...
      let best_of_the_best = Battle.make_play !groups2 group2_size population popsize mapsize in
...
let play () =
  ...
  let p1 = population in
  let p2 = { population with colour = 0xFF0000 } in
